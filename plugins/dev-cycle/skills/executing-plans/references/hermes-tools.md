...
To invoke a dev-cycle skill, use:
...
If `skill_view` cannot find a dev-cycle skill (it may not appear in the catalog
until the plugin fully registers it), fall back to reading the SKILL.md directly:

```
read_file(path="~/.hermes/plugins/dev-cycle/skills/<skill-name>/SKILL.md")
```
